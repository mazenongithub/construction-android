...
import { sortcode, sorttimes, inputUTCStringForLaborID, getScale, returnCompanyList, CreateUser, getEquipmentRentalObj, calculatetotalhours, AmmortizeFactor, calculateTotalMonths, FutureCostPresent, isNumeric, UTCTimefromCurrentDate, sortpart, calculateFloat, getDateInterval, calculateday, calculatemonth, calculateyear, checkemptyobject, getDateTime } from './functions'
import { SaveCompany, SaveProfile, SaveProject, AppleLogin, RegisterUser, LoadAllUsers } from './actions/api';
...
class Construction {
    ...
    getgoogleicon() {
        ...
        if (this.state.width>1200) {
            return ({ width: 373, height: 76 })

        } else if (this.state.width > 600) {

            return ({ width: 295, height: 60 })
        } else {
    ...
    getarrowicon() {
        ...
        const menu = construction.getnavigation.call(this)
        if (menu.open) {
            return ({ width: 36, height: 31 })
        } else {
            return ({ width: 61, height: 53 })
    ...
    async registeruser() {
        ...
        let values = { emailaddress, profile, pass }
        ...
        try {
            ...
            if (response.hasOwnProperty("myuser")) {
                ...
                this.setState({ client: '', clientid: '', emailaddress: '', message: '', emailaddresscheck: false, profilecheck: false, profile: '', firstname: '', lastname: '', profileurl: '', profile: '', password: '' })
    ...
    async loginclient(type) {
        ...
        try {
            ...
            if (response.hasOwnProperty("myuser")) {
                ...
                this.props.reduxUser(response.myuser)
                this.setState({ client: '', clientid: '', emailaddress: '', message: '', emailaddresscheck: false, profilecheck: false, profile: '', firstname: '', lastname: '', profileurl: '', profile: '', password: '', passwordcheck: false })
            } else if (response.hasOwnProperty("message")) {
                this.setState({ message: response.message })
            }
        } catch (err) {
    ...
    validateCompany() {
        ...
        if (company) {
            ...
            if (company.hasOwnProperty("invalid")) {
                validate.validate = false;
                validate.message += company.invalid
    ...
    async saveCompany() {
        ...
        if (checkmanager) {
            ...
            const validate = construction.validateCompany.call(this)
            ...
            alert(`Only Managers can save company `)
        }
    }
    getsavecompanyicon() {
            ...
            if (response.hasOwnProperty("projectids")) {
                ...
                if (response.projectids.hasOwnProperty("proposals")) {
                    ...
                    response.projectids.proposals.map(myproposal => {
                        const o = construction.getproposalkeybyid.call(this, projectid, myproposal.oldproposalid);
                ...
                if (response.projectids.hasOwnProperty("invoices")) {
                    ...
                    response.projectids.invoices.map(myinvoice => {
                        const p = construction.getinvoicekeybyid.call(this, project, myinvoice.oldinvoiceid);
    ...
    validateProject() {
        const construction = new Construction();
        const activeproject = construction.getactiveproject.call(this)
        let project = construction.getprojectbyid.call(this, activeproject.projectid);
        let validate = {};
        validate.validate = true;
        validate.message = "";
        if (project.hasOwnProperty("schedulelabor")) {
    ...
    loginMessage(component) {
        const styles = MyStylesheet();
        return (<View>
            <Text style={[styles.alignCenter, styles.regularFont]}>Login to view {component}.</Text>
        </View>)
    }
}
