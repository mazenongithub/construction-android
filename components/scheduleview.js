...
class ScheduleView {
    ...
    getviewheight() {
        ...
        if(employees.length  + equipment.length > 2) {
            return(210 + ((employees.length  + equipment.length - 2)*120) )
        } else {
            return(210)
        }
    ...
    showschedule(type) {
        ...
        return (
            <View style={{ ...styles.generalContainer, ...styles.bottomMargin15 }}>
                ...
                <Text style={{ ...styles.generalFont, ...regularFont}}>Week of {formatDateStringDisplay(dates.day_1)} - {formatDateStringDisplay(dates.day_7)}</Text>
