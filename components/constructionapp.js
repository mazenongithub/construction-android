...
class ConstructionApp extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            passwordcheck: false, password: '', showpassword: false, activeaccountid: false, showlabor: true, showmaterials: true, showequipment: true, showactuallabor: true, showactualmaterials: true, showactualequipment: true
        }
    ...
    handlelogin() {
        ...
        menu.main = "login";
        this.props.reduxNavigation(menu)
        this.setState({ emailaddress: '', profile: '', password: '', client:'', clientid:'' })
    }
    handleregister() {
        const construction = new Construction();
        let menu = construction.getnavigation.call(this);
        menu.main = "register";
        this.props.reduxNavigation(menu)
        this.setState({ emailaddress: '', profile: '', password: '' })
    }
    handleprofile() {
        const construction = new Construction();
        let menu = construction.getnavigation.call(this);
    ...
    getprojectlinks() {
        ...
        if (myprojects) {
            myprojects.map(project => {
                projectlinks.push(<Text style={[regularFont, styles.alignCenter, styles.bottomMargin10]} onPress={() => { this.makeprojectactive(project.projectid) }} key={project.projectid}>/{project.title}</Text>)
    ...
    render() {
        ...
        const mainBody = () => {
            ...
            const closed_1 = () => {
                if (myuser) {
                    return (<Text style={[styles.alignCenter, headerFont, styles.bottomMargin10]} onPress={() => { this.handleprofile() }}>  /{myuser.profile} </Text>)
                } else {
                    return (<Text style={[styles.alignCenter, headerFont, styles.bottomMargin10]} onPress={() => { this.handlelanding() }}>  / </Text>)
                }
            ...
            const equipment = () => {
                if (checkmanager) {
                    return (<Text style={[styles.alignCenter, regularFont, styles.bottomMargin10]} onPress={() => { this.handleequipment() }}>  /equipment</Text>)
                }
            }
            ...
            const proposals = () => {
                if (checkmanager) {
                    return (<Text style={[styles.alignCenter, regularFont, styles.bottomMargin10]} onPress={() => { this.handleproposals() }}> /proposals</Text>)
                }
            }

            const bidschedule = () => {
                if (checkmanager) {
                    return (<Text style={[styles.alignCenter, regularFont, styles.bottomMargin10]} onPress={() => { this.handlebidschedule() }}> /bidschedule</Text>)
                }
            }
            ...
            const companylinks = () => {
                ...
                if (myuser && menu.open) {
                    if (myuser.hasOwnProperty("company")) {
                        return (
                            <View>
                                <Text style={[styles.alignCenter, regularFont, styles.bottomMargin10]} onPress={() => { this.handleemployees() }}>  /employees</Text>
                                {accounts()}
                                {equipment()}
                                {materials()}
