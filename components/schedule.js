...
class Schedule extends Component {
    ...
    reset() {
        this.setState({ quantity: '', unit: '', unitcost: '', laborrate: '', equipmentrate: '', material: '', equipment: '', milestone:'', employee:'', activelaborid:'', activematerialid:'', activeequipmentid:''})
        this.timeindefault()
        this.timeoutdefault();
    ...
    makematerialactive(materialid) {
        ...
        if (project) {
            ...
            if (this.state.activematerialid === materialid) {
                this.materialdatedefault();
                this.setState({ activematerialid: false, csi_1:'', csi_2:'', csi_3:'', csi_4:'', milestone:''})
                this.reset()
    ...
    render() {
        ...
        if (myuser) {
            if (checkactive) {
                ...
                return (<Text style={{ ...regularFont }}>You have to be active to view this component  </Text>)
            }

        } else {
            return (<Text style={{ ...regularFont }}>Please Login to View Schedule </Text>)
        }
    }
